...
func (p *SynologyProvider) Configure(
	...
	cacheMode := "auto"
	if !data.SessionCacheMode.IsNull() && !data.SessionCacheMode.IsUnknown() && data.SessionCacheMode.ValueString() != "" {
	...
	cachePath := ""
	if !data.SessionCachePath.IsNull() && !data.SessionCachePath.IsUnknown() && data.SessionCachePath.ValueString() != "" {
	...
	if ring != nil {
		if rec, err := readSession(ring, cacheK); err == nil && rec != nil && rec.SessionID != "" && rec.SynoToken != "" {
			if c, ok := cli.(*client.Client); ok {
				c.ImportSession(api.Session{SessionID: rec.SessionID, SynoToken: rec.SynoToken, CreatedAt: rec.IssuedAt})
	...
	v, err, _ := sf.Do(key, func() (any, error) {
		...
		if loginErr == api.ErrOtpRejected && otp_secret != "" {
			wait := nextTotpWaitDuration()
			tflog.Warn(ctx, fmt.Sprintf("Synology OTP rejected — waiting %s before retrying to avoid TOTP replay.", wait))
			resp.Diagnostics.Append(diag.NewWarningDiagnostic(
				"Synology OTP rejected — wait",
				fmt.Sprintf("DSM rejected the OTP (likely replay within the same 30‑second window). Started %s wait before retrying with a fresh code.", wait),
			))
	...
	if mode != "" && mode != "file" && pstr != "" {
		resp.Diagnostics.Append(
			diag.NewAttributeWarningDiagnostic(
				...
				fmt.Sprintf("session_cache is %q, so session_cache_path will be ignored. Set session_cache = \"file\" to use it.", mode),
			),
		)
	}
}
